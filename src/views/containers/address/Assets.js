...
function AssetsPage() {
    ...
    React.useEffect(() => {
        ...
        axios({
            method: 'get',
            url: `${api_get_delegations}${addressid}?verbosity=2`,
        })
            .then(function (response) {
              // console.log(response.data.data.account);
              var totalAmount = 0
              var totalClaimedRewards = 0

              let arr = Object.entries(response.data.data)
              for (let i = 0; i < arr.length; i++) {
                let delegation = arr[i][1]
                totalAmount = totalAmount + parseInt(delegation.amount)
                totalClaimedRewards = totalClaimedRewards + parseInt(delegation.claimedRewards)
              }

              setDelegatedAmount(totalAmount / e18)
              setClaimedRewards(totalClaimedRewards / e18)
            });
            axios({
                method: 'get',
                url: `${api_get_price}`,
